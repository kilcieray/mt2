    ...
    try:
        ...
        if req.status_code == 200:
            ...
            for vnum_str, name in matches:
                ...
                known_vnums.add(vnum)
                options.append(f"{clean_name} (Vnum: {vnum})")
...
def get_vnum_range(target_vnum):
    """Hedef vnumdan başlayıp, koddaki bir sonraki item vnumuna kadar olan aralığı belirler."""
    if not KNOWN_VNUMS:
        return target_vnum, target_vnum + 9 
    
    for v in KNOWN_VNUMS:
        if v > target_vnum:
            return target_vnum, v - 1
...
def migrate_filters(filters):
    for f in filters:
        ...
        if 'vnum' not in f:
            f['vnum'] = 0
            if f.get('name') and f['name'].isdigit():
                f['vnum'] = int(f['name'])
        if 'label' not in f:
            f['label'] = f.get('name', 'İsimsiz Filtre')
    return filters
...
with st.sidebar:
    ...
    if len(KNOWN_VNUMS) > 0:
        st.success(f"🌐 İnternetten **{len(KNOWN_VNUMS)}** adet eşyanın ismi ve Vnum bilgisi sisteme başarıyla çekildi.")
    else:
        st.warning("⚠️ metin2cms sitesine şu an ulaşılamıyor. Sadece manuel olarak rakam girerek arama yapabilirsiniz.")
...
with t1:
    ...
    st.markdown("---")
    if st.button("➕ YENİ VNUM TAKİBİ EKLE", type="primary"):
    ...
    for i, item in enumerate(st.session_state.track_list):
        ...
        with st.expander(display_title, expanded=True):
            ...
            selected_opt = c1.selectbox("Eşya İsmine Göre Ara (Örn: Buz Tılsımı)", options=ITEM_OPTIONS, index=current_index, key=f"sel_{i}")
            
            if selected_opt != ITEM_OPTIONS[0]:
                auto_vnum = int(re.search(r'\(Vnum: (\d+)\)', selected_opt).group(1))
                auto_label = selected_opt.split(" (Vnum")[0]
                
                new_v = c2.number_input("Hedef Vnum", value=auto_vnum, key=f"vn_{i}", min_value=0)
                new_l = auto_label
            else:
                new_v = c2.number_input("Hedef Vnum (Elle Girin)", value=item.get('vnum', 0), key=f"vn_{i}", min_value=0)
                new_l = item.get('label', 'Manuel Eşya')
            
            new_w = c3.number_input("Maks. Won", value=item.get('max_won', 3000), key=f"w_{i}", min_value=0)
            
            if new_v != item.get('vnum') or new_w != item.get('max_won'):
                item['vnum'] = new_v
                item['label'] = new_l
                item['max_won'] = new_w
                save_to_db(st.session_state.track_list)
            ...
            if item.get('vnum', 0) > 0:
                st.caption(f"🔍 **Otomatik Taranacak Vnum Aralığı:** {min_v} ile {max_v} arası.")
