...
with t1:
    ...
    c_exp.download_button(
        label="📥 FİLTRELERİ DIŞA AKTAR (YEDEKLE)",
        data=filters_json,
        file_name=f"pazar_filtre_yedek_{datetime.now().strftime('%d_%m_%Y')}.json",
        mime="application/json",
        use_container_width=True
    )

    uploaded_file = c_imp.file_uploader("📤 YEDEK DOSYASI YÜKLE", type=['json'], label_visibility="collapsed")
    if uploaded_file is not None:
        try:
            imported_data = json.load(uploaded_file)
            if st.button("✅ YÜKLEMEYİ ONAYLA", use_container_width=True):
                st.session_state.track_list = imported_data
                save_to_db(imported_data)
                st.success("Filtreler başarıyla yüklendi!")
                st.rerun()
        except:
...
with t2:
    ...
    else: st.info("Henüz filtre eklenmedi.")
