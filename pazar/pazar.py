...
STATE_FILE = "bot_state.json" # Botun çalışıp çalışmadığını buraya yazacağız
...
# Botun çalışma durumunu dosyadan kontrol et (Thread güvenliği için)
...
with st.sidebar:
    ...
    if not is_running:
        if st.button("▶️ BAŞLAT", use_container_width=True, type="primary"):
            set_bot_status(True)
            thread = threading.Thread(target=tracker_worker, daemon=True)
            thread.start()
            st.rerun()
    else:
