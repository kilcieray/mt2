        ...
        try:
            ...
            for m_item in data:
                ...
                for crit in active_filters:
                    # Ana arama kutusu kontrolü (Boş mu, Sayı mı, Harf mi?)
                    filter_text = crit.get('name', '').strip().lower()
                    
                    is_match = False
                    if not filter_text:
                        is_match = True  # Kutu boşsa isme/vnuma bakmadan devam et
                    elif filter_text.isdigit():
                        is_match = (m_vnum == int(filter_text)) # Sayıysa Vnum kontrolü
                    else:
                        is_match = (filter_text in m_name) # Harf varsa İsim kontrolü
                        
                    if is_match and m_item['wonPrice'] <= crit['max_won']:
                        match_count = 0
...
with t1:
    ...
    for i, item in enumerate(st.session_state.track_list):
        with st.expander(f"📦 Filtre #{i+1}: {item['name'] if item['name'] else 'Yeni İtem'}", expanded=True):
            ...
            new_n = c1.text_input("İtem Adı veya Vnum", item['name'], key=f"n_{i}", placeholder="Örn: 55712 veya Kılıç")
            ...
            for j, block in enumerate(item.get('blocks', [])):
                ...
                if current_type not in BLOCK_TYPES: current_type = 'Efsun' # Eski yedeği düzeltme
