...
# Göz yormayan koyu tema CSS'i (Eski kodla uyumlu)
...
def load_from_db():
    ...
    try:
        with open(DB_FILE, "r", encoding="utf-8") as f:
            ...
            # --- ÇÖKME HATASINI ÖNLEYEN KURTARICI ---
            fixed_data = []
            for item in data:
                fixed_item = {
                    "name": item.get("name", item.get("label", "")),
                    "max_won": item.get("max_won", 3000),
                    "attrs": item.get("attrs", [])
                }
                # Eğer yeni sistemden kalma "blocks" varsa eski "attrs" sistemine çevir
                if "blocks" in item and not fixed_item["attrs"]:
                    for b in item["blocks"]:
                        if b.get("type") == "Efsun" or "id" in b:
                            fixed_item["attrs"].append({"id": b.get("id", 72), "val": b.get("val", 0)})
        ...
        try:
            ...
            for m_item in data:
                ...
                for crit in active_filters:
                    ...
                    if crit_name.lower() in m_name and m_item['wonPrice'] <= crit.get('max_won', 3000):
                        match_count = 0
                        
                        crit_attrs = crit.get('attrs', [])
                        for req in crit_attrs:
                            for p_id, p_val in combined_attrs:
                                if p_id == req.get('id') and p_val >= req.get('val', 0):
                                    match_count += 1; break
                                    
                        if match_count == len(crit_attrs):
...
with st.sidebar:
    ...
    if not is_running:
        ...
        if st.button("⏹️ DURDUR", use_container_width=True):
            ...
            st.rerun()
    st.divider()
    st.caption(f"📊 Durum: {'Çalışıyor' if is_running else 'Durdu'}")
...
with t1:
    ...
    if up_file:
        try:
            ...
            if st.button("✅ YEDEĞİ UYGULA", use_container_width=True):
                # Yedeği uygularken de güvenli kontrolden geçir
                fixed_data = []
                for item in data:
                    fixed_item = {
                        "name": item.get("name", item.get("label", "")),
                ...
                st.session_state.track_list = fixed_data
                save_to_db(fixed_data)
                st.success("Yükleme Başarılı!"); st.rerun()
        except: st.error("Dosya Hatalı!")

    st.divider()
    if st.button("➕ Yeni İtem Takibi"):
    ...
    for i, item in enumerate(st.session_state.track_list):
        # Arayüz çökmelerini önlemek için .get() kullanıyoruz
        display_name = item.get('name', '')
        with st.expander(f"Filtre #{i+1}: {display_name}", expanded=True):
            c1, c2, c3 = st.columns([3, 1, 0.5])
            new_n = c1.text_input("İsim", display_name, key=f"n_{i}")
            new_w = c2.number_input("Won", value=item.get('max_won', 3000), key=f"w_{i}")
            if new_n != item.get('name') or new_w != item.get('max_won'):
            ...
            for j, attr in enumerate(item.get('attrs', [])):
                ec1, ec2, ec3 = st.columns([3, 1, 0.5])
                curr_l = next((k for k, v in EFSUN_OPTIONS.items() if v == attr.get('id', 72)), SORTED_EFSUNS[0])
                new_ef = ec1.selectbox("Efsun", SORTED_EFSUNS, index=SORTED_EFSUNS.index(curr_l), key=f"e_{i}_{j}")
                new_val = ec2.number_input("Min", value=attr.get('val', 0), key=f"v_{i}_{j}")
                if EFSUN_OPTIONS[new_ef] != attr.get('id') or new_val != attr.get('val'):
            ...
            if st.button("➕ Efsun", key=f"ae_{i}"):
                if 'attrs' not in item: item['attrs'] = []
                item['attrs'].append({"id": 72, "val": 0}); save_to_db(st.session_state.track_list); st.rerun()

with t2:
    if st.session_state.track_list: st.table(st.session_state.track_list)
    else: st.info("Filtre yok.")
